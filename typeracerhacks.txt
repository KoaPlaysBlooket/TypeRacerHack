DO NOT COPY THIS
yes you can edit the speed at "3)||3)/3 | 3" edit numbers only  
COPY THIS: 
...
function a () {
    ...
    setTimeout(a, (Math.max(Math.min(250,60000/Array.from($('.rankPanelWpm')).map(x => parseInt(x.innerText)).sort((a, b)=>b-a)[1]/5), 3)||3)/3 | 3)*Math.random()
}
a();
